...
exports.todayRepeatTasks = async (ctx, next) => {
...
exports.todayNoRepeatTasks = async (ctx, next) => {
    // 用户下，所有的项目
    const programRes = await db.exec('select * from program where user_id=?', [ctx.state.account.user_id]);
    ...
    if (!programRes.error) {
        ...
        if (!taskRes.error) {
            ...
            const todayEnd = moment(moment(new Date()).format('YYYY-MM-DD 24:00:00'));
            //找出所有重复的任务。和不重复，时间是今天的任务
            let noRepeatResult = tasks.map(item => {
                item.user_id = ctx.state.account.user_id;
            ...
            noRepeatResult = noRepeatResult.filter(item => !!item).sort()
            ctx.body = JSON.stringify({
                success: true,
